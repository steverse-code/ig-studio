...
grep -l "\"pillar\": \"<오늘 필러>\"" content/<오늘날짜>-*.json 2>/dev/null
```

오늘 날짜 + 오늘 필러 조합이 이미 `content/` 에 있고 그 슬러그가 `queue.json` 에서
`published` 이거나 `pending` 이면, **이 슬롯은 이미 처리된 것이다.**
새 글을 만들지 말고 "슬롯 이미 채워짐 — 건너뜀"이라고 보고하고 즉시 종료한다.

(오전·오후가 같은 필러인 날은 없으므로 날짜+필러 조합으로 슬롯이 특정된다.
목요일 오전 success / 일요일 오후 success 처럼 요일이 다르면 날짜가 달라 충돌하지 않는다.)
...
**논문 PDF 원문을 읽는 요령** — 이 실행 환경에는 `pdftoppm` 이 없어서 Read 툴의 PDF
렌더링이 실패하고, WebFetch 의 PDF 요약도 압축 스트림을 못 읽어 수치를 놓치는 경우가 있다.
그럴 때는 PDF 를 받아 파이썬 `zlib` 으로 텍스트 스트림을 직접 풀어서 읽는다.
2차 요약 기사의 숫자를 그대로 믿지 말고 이 방법으로 원문을 대조하라.
